...
lui, l'extraction ne produit rien. Le skill le vérifie en premier.
...
Copiez le skill lui-même dans vos skills personnels — seuls `SKILL.md`,
...
Pour une équipe, placez le dossier dans `.claude/skills/` d'un dépôt : le skill
devient disponible pour tous ceux qui y travaillent.

## Utilisation

Une fois installé, demandez simplement :
...
La partie intéressante n'est pas ce skill en particulier, c'est la méthode
...
Claude Code fournit un skill `skill-creator` qui structure le travail :
`SKILL.md` pour le déroulé, `scripts/` pour le code, `references/` pour ce qui ne
tient pas dans le contexte à chaque fois.

## Ce qui fait qu'un skill vaut mieux qu'un prompt
...
Le skill fait le bon choix sur n'importe quel jeu de données.
...
Le skill envoie des **coordonnées GPS et des horodatages** à
[Open-Meteo](https://open-meteo.com/) pour reconstituer la météo. Les
coordonnées de départ d'une sortie révèlent souvent un domicile. Le skill demande
